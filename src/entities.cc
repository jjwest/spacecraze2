...
int angle_in_degrees(Point from, Point to) {
...
void Player::update(std::vector<Laser> *player_lasers,
    ...
    { // update movement
        ...
        bool can_move_down =
            position.y + position.height + speed <= g_screen_height;
    ...
    { // Shoot
        ...
        if (left_mouse_button_pressed &&
            ...
            Point origin;
            // We hack the positions a bit to ensure that the laser originates from a
            // good position relative to the player ship texture
    ...
    angle = angle_in_degrees(position, mouse_position);
}
...
void Drone::update(Point destination) {
    auto [dx, dy] = calculate_movement_deltas(position, destination);
    position.x += dx * speed;
    position.y += dy * speed;
    angle = angle_in_degrees(position, destination);
}

bool at_edge_of_screen(const Rectangle &rect) {
    return (rect.x <= 0 || rect.x + rect.width >= g_screen_width || rect.y <= 0 ||
            rect.y + rect.height >= g_screen_height);
}

void Blaster::update(std::vector<Laser> *lasers, Point player_pos) {
    if (at_edge_of_screen(position)) {
    ...
    angle = angle_in_degrees(position, player_pos);
    ...
    { // Shoot
        ...
        if (current_time - time_last_shot > shoot_cooldown_ms) {
            ...
            auto angle = angle_in_degrees(origin, player_pos);
...
void Blaster::update_movement_direction() {
...
Laser::Laser(Point origin, Point destination, float damage, int angle)
    : angle{angle}, damage{damage} {
        ...
        auto [dx, dy] = calculate_movement_deltas(origin, destination);
        this->dx = dx;
        this->dy = dy;
                    }
